...
import type { DashboardRecord } from "@/app/components/dashboard/DashboardProvider";
import { updateRoomPrice } from "@/lib/firestore/rooms";
...
export default function PricingPage() {
  ...
  const handleSave = async (room: DashboardRecord) => {
    try {
      ...
      <section className="max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="mb-14">
          ...
          <p className="text-white/45 mt-5 leading-8 max-w-3xl">
            Configure room pricing, review platform deductions,
            taxation, and estimated partner payouts.
          </p>
        </div>
        ...
        {!isLoading && rooms.length === 0 && (
          <div className="border border-dashed border-white/10 rounded-[35px] p-16 text-center bg-white/[0.02]">
            ...
            <p className="text-white/45 max-w-xl mx-auto leading-8">
              Add rooms first to begin configuring luxury pricing
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {rooms.map((room) => {
            ...
            const razorpayFee = Math.round(
              basePrice * 0.025
            );

            const finalGuestPrice =
              basePrice + gst + platformFee + razorpayFee;

            const partnerReceives =
              basePrice - platformFee - razorpayFee;
              ...
              >
                ...
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="w-16 h-16 rounded-2xl bg-[#d4a574]/10 border border-[#d4a574]/20 flex items-center justify-center">
                    <IndianRupee className="text-[#d4a574]" />
                  </div>
                </div>
                ...
                <div className="mb-8">
                  ...
                  <input
                    type="number"
                    value={room.price as string | number}
                    onChange={(e) =>
                      handlePriceChange(
                        room.id,
                        e.target.value
